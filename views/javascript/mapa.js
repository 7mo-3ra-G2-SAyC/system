...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const pathResult = resultMap.querySelectorAll('path')
    ...
    const cargarMapa = (card) => {
        ...
        switch(card.id){
            case "piso1Span":    
                ...
                plantaUnoSvg.querySelectorAll('path').forEach((path) => {
                    const [ piso, nroSala ] = path.id.split('_')
                    ...
                    estadosSalas.pisoUno.map((sala) => {
                        ...
                        if (`sala${sala.sala}` == nroSala) {
                            ...
                            switch (sala.estado) {
                                case "libre":
                                    path.style.fill = "#9ACC77";
                                    break;
            
                                case "ocupado":
                                    path.style.fill = "#F0D442";
                                    break;
            
                                case "cerrado":
                                    path.style.fill = "#CC2649";
                                    break;
                            }
                        }
            ...
            case "plantaBajaSpan":    
                ...
                plantaBajaSvg.querySelectorAll('path').forEach((path) => {
                    const [ piso, nroSala ] = path.id.split('_')
            ...
            case "piso2Span":    
                ...
                plantaDosSvg.querySelectorAll('path').forEach((path) => {
                    const [ piso, nroSala ] = path.id.split('_')
                    
                    estadosSalas.pisoDos.map((sala) => {
            ...
            case "tallerSpan":    
                ...
                tallerSvg.querySelectorAll('path').forEach((path) => {
                    const [ piso, nroSala ] = path.id.split('_')
                    
                    estadosSalas.taller.map((sala) => {
    ...
    function getTiempoRestante() {
        ...
        const fechaProxima = new Date('2024-11-11 21:40:00');
            ...
            if (`sala${sala.sala}` == nroSala) {
                ...
                switch (sala.estado) {
                    ...
                    case "cerrado":
                        ...
                        estadoCardFuncion.innerText = "Sala Cerrada";
                        tituloCardFuncion.innerText = sala.name;
                        ubicacionCardFuncion.innerText = getUbicacion(piso, sala.sala);
